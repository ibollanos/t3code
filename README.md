...
SSH environments (**Add Environment → SSH**) also work in fork builds: the desktop installs the
server on the remote host from this fork's release tarball
(`releases/download/fork-v{version}/t3-{version}.tgz`) instead of npm, where the fork's version
does not exist.
